...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(AuthenticationManagerBuilder auth) throws Exception {
        auth.userDetailsService(userConfig)//用户认证处理
                .passwordEncoder(passwordConfig);//密码处理
    }
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http
                ...
                //定制url访问权限
                .authorizeRequests()
                .antMatchers("/bootstrap/**","/sys/**","/layui/**", "/css/**", "/js/**", "/images/**", "/webjars/**", "/getVerifyCodeImage").permitAll()
                //系统相关、非业务接口只能是管理员以上有权限，例如获取系统权限接口、系统用户接口、系统菜单接口、以及用户与权限、菜单关联接口
                .antMatchers("/sys/user/**", "/sys/authority/**", "/sys/menu/**").hasAnyAuthority("ROLE_ADMIN", "ROLE_SA")
                //admin接口测试
