 ...
 * @return {Boolean|Number} false when not found, or the extra
 */
...
inherit(PointerEventInput, Input, {
    ...
    handler: function PEhandler(ev) {
        ...
        // get extra of the event in the store
...
+ function($) {
  ...
  //如果参数过多，建议通过 constant 对象进行配置，而不是传入多个参数。
...
+ function($) {
  ...
  var Picker = function (params) {
      ...
      function onPickerClose() {
          p.opened = false;
          if (p.input && p.input.length > 0) p.input.parents('.wxpage-content').css({'padding-bottom': ''});
...
+function ($) {
  ...
  var Calendar = function (params) {
      ...
      function openOnInput(e) {
          ...
          p.open();
          if (p.params.scrollToInput && !isPopover()) {
              var pageContent = p.input.parents('.wxpage-content');
              if (pageContent.length === 0) return;
