 ...
 * @param {Function} iterator 对每个数组元素进行调用的函数，该函数有两个参数，第一个为数组元素，第二个为数组索引值，function (item, extra)。
 ...
 * @constant {Object} vars 创建imageUploader时所需要的参数
 * @constant {Number} vars.gridWidth 每一个预览图片所占的宽度，应该为flash寛的整除
 * @constant {Number} vars.gridHeight 每一个预览图片所占的高度，应该为flash高的整除
 * @constant {Number} vars.picWidth 单张预览图片的宽度
 * @constant {Number} vars.picHeight 单张预览图片的高度
 * @constant {String} vars.uploadDataFieldName POST请求中图片数据的key,默认值'picdata'
 * @constant {String} vars.picDescFieldName POST请求中图片描述的key,默认值'picDesc'
 * @constant {Number} vars.maxSize 文件的最大体积,单位'MB'
 * @constant {Number} vars.compressSize 上传前如果图片体积超过该值，会先压缩
 * @constant {Number} vars.maxNum:32 最大上传多少个文件
 * @constant {Number} vars.compressLength 能接受的最大边长，超过该值会等比压缩
 * @constant {String} vars.url 上传的url地址
 * @constant {Number} vars.mode mode == 0时，是使用滚动条，mode == 1时，拉伸flash, 默认值为0
 ...
 * @constant {Object} createOptions 创建flash时需要的参数，请参照baidu.swf.create文档
 * @constant {String} createOptions.width
 * @constant {String} createOptions.height
 * @constant {Number} maxNum 最大可选文件数
 * @constant {Function|String} selectFile
 * @constant {Function|String} exceedMaxSize
 * @constant {Function|String} deleteFile
 * @constant {Function|String} uploadStart
 * @constant {Function|String} uploadComplete
 * @constant {Function|String} uploadError
 * @constant {Function|String} uploadProgress
 */
 ...
 * @constant {String} [charset] script的字符集
 * @constant {Integer} [timeOut] 超时时间，超过这个时间将不再响应本请求，并触发onfailure函数
 * @constant {Function} [onfailure] timeOut设定后才生效，到达超时时间时触发本函数
 * @remark
 ...
 * @constant {string} [charset] script的字符集
 * @constant {string} [queryField] 服务器端callback请求字段名，默认为callback
 * @constant {Integer} [timeOut] 超时时间(单位：ms)，超过这个时间将不再响应本请求，并触发onfailure函数
 * @constant {Function} [onfailure] timeOut设定后才生效，到达超时时间时触发本函数
 * @remark
