package com.code.demo.api.aspect;
...
public class AuthorizeAspect {
     ...
     * 拦截接口除了登录/注册接口
     */
    @Pointcut("execution(public * com.code.demo.api.controller.*Controller.*(..))" +
            "&& !execution(public * com.code.demo.api.controller.UserController.userLogin(..))" +
            "&& !execution(public * com.code.demo.api.controller.UserController.userRegister(..))" )
        ...
        if (!"dev".equals(env)) { //开发环境不进行auth认证
            try {
                if (JWTUtil.parseJWT(request.getHeader(tokenName)) == null) {
                    log.warn("签名认证失败，请求接口：{}，请求IP：{}，请求参数：{}", request.getRequestURI(), getIpAddress(request), JSON.toJSONString(request.getParameterMap()));
